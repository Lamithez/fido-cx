...
impl From<AgileHpkeError> for String {
    fn from(value: AgileHpkeError) -> Self {
        match value {
            AgileHpkeError::AlgMismatch(a, b) => {
                format!("算法不匹配：{:?}，{:?}", a, b)
            }
            AgileHpkeError::UnknownAlgIdent(a, b) => {
                format!("未知的算法标识符：{:?}，{:?}", a, b)
            }
            AgileHpkeError::HpkeError(s) => {
                format!("HPKE错误：{:?}", s)
            }
        }
    }
...
pub struct AgileOpModeR<'a> {
    pub(crate) kem_alg: KemAlg,
    pub(crate) op_mode_ty: AgileOpModeRTy<'a>,
}
...
pub struct AgileOpModeS<'a> {
    pub(crate) kem_alg: KemAlg,
    pub(crate) op_mode_ty: AgileOpModeSTy<'a>,
}
